...
export const shuffle = (poolSize = 16, cardSize = 16, last?: number[]) => {
  let card = [...Array(poolSize).keys()];
  do {
    for (let i = card.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [card[i], card[j]] = [card[j], card[i]];
    }
    card = card.slice(0, cardSize);
  } while (
    last &&
    card.length === last.length &&
    card.every((item, i) => item === last[i])
  );
  return card;
};
export const cleanName = (name: unknown) =>
  String(name ?? "")
    ...
    .replace(/\s+/g, " ")
    .slice(0, 24);
export const isExpired = (last: string) =>
...
export async function transferHost(roomId: string) {
  ...
  const { data: host } = await db
    ...
    .maybeSingle();
  if (!host || host.last_seen_at < cutoff) {
...
export async function roomPayload(roomCode: string, player: { id: string }) {
  ...
  const found = await identify(
    roomCode,
    player.id,
    (player as { access_secret?: string }).access_secret,
  );
  if (!found?.player) throw new Error("You are not a member of this room.");
  const [{ data: players }, { data: card }] = await Promise.all([
    ...
    db
      ...
      .eq("round_number", found.room.round_number)
      .single(),
  ]);
  return {
    room: found.room,
    ...
    me: {
      id: found.player.id,
      display_name: found.player.display_name,
      progress_count: found.player.progress_count,
      has_bingo: found.player.has_bingo,
      is_active: found.player.is_active,
      joined_at: found.player.joined_at,
      last_seen_at: found.player.last_seen_at,
    },
    card,
  };
}
