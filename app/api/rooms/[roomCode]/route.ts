  ...
  try {
    ...
    const [{ data: players }, { data: card }] = await Promise.all([
      db
        .from("players")
        .select(
          "id,display_name,progress_count,has_bingo,is_one_away,bingo_count,total_marks,near_miss_count,is_active,joined_at,last_seen_at",
        )
        .eq("room_id", room.id)
        .order("joined_at"),
      db
